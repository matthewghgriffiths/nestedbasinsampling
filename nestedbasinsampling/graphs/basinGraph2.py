...
from nestedbasinsampling.utils import (
    len_iter, SortedCollection, GraphError)
...
#        for child in self.graph.successors(gbasin):
#            children = self.graph.successors(child)
#            if len(children) == 1:
#                mbasin, = children
#                m, = mbasin.minima
#                lh = (0.5*self.dof*np.log(child.energy - m.energy)
#                  - 0.5*m.fvib - np.log(m.pgorder))
#                ns = self.graph[gbasin][child]['run'].nlive
#                weight = 1./(polygamma(1,ns) - polygamma(1,ns+1)).sum()
#                child.log_vol = (
#                    self.graph[gbasin][child]['log_vol'][-1]
#                    - np.log(self.count_runs(child)))
#                diff = child.log_vol - lh
#                diffs[child] = diff
...
#                        "BasinGraph > harmonic interpolation, basinE="+
#                        "{:10.5g}, log vol diff"+
#                        "{:10.5g}").format(child.energy, diff)
#                    print

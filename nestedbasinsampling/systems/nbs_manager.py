...
from ..utils import LOG_CONFIG, Result
from ..storage import Database, Replica, Run
from ..concurrent import RemoteManager, BaseManager, utils

logger = logging.getLogger('nbs.nbs_manager')
...
class NBS_manager(BaseManager):
    ...
    def get_job(self):
        ...
        rep = self.g_replica
        return 'nopt', (rep.coords, rep.energy, self.nbs_system.stepsize), {}
    ...
    def __call__(self, job):
        ...
        return job, getattr(self.nbs_system, method)(*args, **kwargs)

    @classmethod
    def main(cls, system, args):
        options, args = utils.parse_args(args)
        ...
        manager = cls(system, database=database, max_iter=niter)

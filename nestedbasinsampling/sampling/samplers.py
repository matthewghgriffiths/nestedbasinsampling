...
from nestedbasinsampling.utils import Result, SamplingError
from nestedbasinsampling.structure.constraints import BaseConstraint
from nestedbasinsampling.sampling.takestep import AdaptiveStepsize, TakestepHyperSphere, hypersphere_step
...
class DetectStep(object):
    ...
    def adjust_stepsize(self, naccept, nreject, stepsize=None):
        if stepsize is None:
            stepsize = self.stepsize

        return stepsize * self.factor_step**(naccept - self.rel_ratio*nreject)
    ...
    def detect(self, Ecut, coords, stepsize=None, nsteps=None, nadapt=None):
        ...
        for i in xrange(nadapt):
            try:
                newcoords, Enew, naccept, nreject, newstepsize = \
            ...
            stepsize = self.adjust_stepsize(naccept, nreject,
                                            stepsize=stepsize)
...
class GalileanSampler(BaseSampler):
    ...
    def __init__(self, pot, genstep=hypersphere_step, constraint=None,
                 nsteps=100, stepsize=0.1, factor=0.2,
                 acc_ratio=0.5, step_factor=1.1, frequency=50,
        ...
        self.rel_ratio = self.acc_ratio/(1. - self.acc_ratio)
        self.factor_step = self.step_factor**(1./self.frequency)
        self.accept_factor = self.factor_step
        self.reject_factor = self.factor_step**(-self.rel_ratio)
...
class GMCSampler(GalileanSampler):
    ...
    def new_point(self, Ecut, coords, nsteps=None, stepsize=None, depth=0):
        ...
        #stepsize = np.linalg.norm(coords)*.02
        ...
        while(ns < nsteps):
            ...
            if Eaccept and constraint:
                ...
                if Eaccept and constraint:
                    ...
                    if not np.isclose(Enew, Ecut):
                        raise
                        print "HCM > reflect fail", Enew, Efail, Ecut, stepsize
...
#            if ns % self.frequency == 0:
#                fact = self.factor_step**(na - self.rel_ratio*nr)
#                stepsize *= fact
#                p *= fact
#                na = 0
#                nr = 0
...
#        print "{:8.4g}, {:3d}, {:3d}".format( 1.-1.*nreject/nsteps, nreject, naccept)
#        print "\t", istep, stepsize/np.linalg.norm(coords)
#        print "\t", "{:8.4g}, {:8.4g}".format(Enew, Ecut)

        return newcoords, Enew, nsteps, nreject, nreflect
...
if __name__ == "__main__":
    ...
    gsampler = GMCSampler(pot, stepsize=0.5)
    samples = []
    Es = []

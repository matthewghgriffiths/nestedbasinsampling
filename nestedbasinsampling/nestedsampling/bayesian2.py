...
def _estimate_beta_parameters(X, resp):
    """
    Parameters
    ----------
    ...
    betas = np.einsum("ij,ikl->jkl", resp, X) / nk[:, None, None]
    return nk, betas

def _estimate_log_beta_prob(pcounts, betas):
    ...
    means : array-like, shape (n_components, n_features, 2)
    ...
    return (betaln(*(pcounts[None,...]+betas[:,None,...]-1).T)
            - betaln(*pcounts[None,...].T)
            - betaln(*betas[:,None,...].T)).sum(0)
...
class BayesianBetaMixture(BaseMixture):
    ...
    def _check_is_fitted(self):
        check_is_fitted(self, ['weight_concentration_', 'betas_'])
    ...
    def _estimate_betas(self, nk, ab):
        """Estimate the parameters of the Dirichlet distribution.
        Parameters
        ----------
        nk : array-like, shape (n_components,)
        ab : array-like, shape (n_components, n_features, 2)
        """
        self.betas_ = nk[:,None,None] * ab + self.beta_prior
    ...
    def _m_step(self, X, log_resp):
        ...
        """
        nk, bk = _estimate_beta_parameters(X, np.exp(log_resp))
    ...
    def _estimate_log_prob(self, X):
        log_pcount = _estimate_log_beta_prob(X, self.betas_)
        return log_pcount
    ...
    def _compute_lower_bound(self, log_resp, log_prob_norm):
        ...
        log_beta = _estimate_log_beta_prob(self.betas_, prior).sum()
    ...
    def _get_parameters(self):
        return (self.weight_concentration_, self.betas_)

    def _set_parameters(self, params):
        (self.weight_concentration_, self.betas_) = params

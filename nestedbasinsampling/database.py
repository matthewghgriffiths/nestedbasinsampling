...
class NestedSamplingRun(Base):
    ...
    Nremove = deferred(Column(PickleType))
    ...
    def __init__(self, Emax, Nlive, Nremove, configs=None):
        self.Emax = Emax
        self.Nlive = Nlive
    ...
    def __eq__(self, run):
        ...
        if isinstance(run, NestedSamplingRun):
...
class IntervalCommit(object):
    ...
    def __init__(self, db, func, commit_interval=10):
        self.db = db
        self.func = func
...
#         self._check_schema_version_and_create_tables(newfile)
    ...
    def runs(self):
        return self.session.query(NestedSamplingRun).all()

    def add_run(self, Emax, Nlive, Nremove, configs=None, commit=True):
        ...
        new = NestedSamplingRun(np.asanyarray(Emax),
                                np.asanyarray(Nlive),
                                np.asanyarray(Nremove),
                                configs)
    ...
    def get_run(self, runid):
        ...
        return  self.session.query(NestedSamplingRun).get(runid)

    def update_run(self, run, Emax, Nlive, Nremove, configs=None, commit=True):
        """
        Parameters
        ----------
        ...
        runid = run.id() if isinstance(run, NestedSamplingRun) else run
        ...
        run.Nremove = np.asanyarray(Nremove)
        run.configs = configs
    ...
    def addMinimumRes(self, res, commit=True):
        """add a new minimum to database from a Result object

        any extra fields in the Result object other than
        dict((k, v) for k, v in res.iteritems() if k not in ['rms','success','energy','coords','message','grad','nsteps'])
        ...
        m = Minimum(res.energy, res.coords)
        m.user_data = dict((k, v) for k, v in res.iteritems() if k not in
            ['rms','success','energy','coords','message','grad','nsteps'])
    ...
    def minimumRes_adder(self, interval=None):
        interval = self.commit_interval if interval is None else interval
        return IntervalCommit(self, self.addMinimumRes, commit_interval=interval)
